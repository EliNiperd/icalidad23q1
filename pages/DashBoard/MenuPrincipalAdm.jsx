...
export const MenuPrincipalAdm = ({ activeMenuId, menuId }) => {
  const classDivMenu =
    'flex flex-col text-4xl text-accent hover:bg-neutral-200 rounded-lg pr-2';
  ...
  return (
    <div>
      <div className="flex flex-col content-between gap-2 pt-4 menu transition-all duration-500">
        <h1 className="hidden">
          {menuDataMenu.idMenu ? menuDataMenu.idMenu : 0}
        </h1>

        <MenuItem
          icon="AiFillHome"
          className={classIconMenu}
          classMenu={classDivMenu}
          classH1Menu={classH1Menu}
          classSelected={classSelected}
          href={href}
          handleMenuItemIClick={handleMenuItemIClick}
          idMenu={idMenu}
          MenuDataidMenu={menuDataMenu.idMenu}
          NameMenu="iCalidad"
        />
          ...
          <Link key="1" href="/DashBoard" passHref>
            <a
              onClick={(event) => {
                ...
                handleMenuItemIClick(149);
              }}
              className={menuDataMenu.idMenu === 149 ? classSelected : ''}
            >
        ...
        <div className={classDivMenu}>
          <Link key="2" href="/AdminDocument" passHref>
            <a
              onClick={(event) => {
                event.stopPropagation();
                handleMenuItemIClick(150);
              }}
              className={menuDataMenu.idMenu === 150 ? classSelected : ''}
            >
              <FcAnswers className={classIconMenu}></FcAnswers>
        ...
        <div className={classDivMenu}>
          <Link key="3" href="/AdminAction">
            <a
              onClick={() => handleMenuItemIClick(151)}
              className={menuDataMenu.idMenu === 151 ? classSelected : ''}
            >
              <FcCollaboration className={classIconMenu}></FcCollaboration>
        ...
        <div className={classDivMenu}>
          <Link key="4" href="/Todo">
            <a
              onClick={() => handleMenuItemIClick(152)}
              className={menuDataMenu.idMenu === 152 ? classSelected : ''}
            >
              <FcSurvey className={classIconMenu}></FcSurvey>
        ...
        <div className={classDivMenu}>
          <Link key="5" href="/Todo">
            <a
              onClick={() => handleMenuItemIClick(4)}
              className={menuDataMenu.idMenu === 153 ? classSelected : ''}
            >
              <FcConferenceCall className={classIconMenu}></FcConferenceCall>
        ...
        <div className={`${classDivMenu} mb-2`}>
          <Link key="6" href="/Todo">
            <a
              onClick={() => handleMenuItemIClick(4)}
              className={menuDataMenu.idMenu === 154 ? classSelected : ''}
            >
              <AiFillSetting className={classIconMenu}></AiFillSetting>
