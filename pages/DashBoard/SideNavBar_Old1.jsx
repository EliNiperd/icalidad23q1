...
export function  ElementMenu1  (key){
    const classVar = " inline-block text-4xl text-[#30A9ED] group-hover:text-white ";
    ...
    switch (key.description) {
        case 'iCalidad':
              return (
                <Link key={key.urlMenu} href={key.urlMenu}  >
                    <a  >
                    <AiFillHome key={key.idMenu} className={classVar}>
                    </AiFillHome>
                    {key.description}
                    </a>
                </Link>
        ...
        case 'Poder Documental':
              return(
                <Link key={key.urlMenu} href={key.urlMenu}  >
                    <a >
                    <FcAnswers key={key.idMenu} className={classVar}>
                    </FcAnswers>
                    {key.description}
                    </a>
                </Link>
        ...
        case 'Acciones':
              return(
                <Link key={key.urlMenu} href={key.urlMenu} >
                    <a>
                    <FcCollaboration key={key.idMenu} className={classVar} > 
                    </FcCollaboration>
                    {key.description}
                    </a>
                </Link>
        ...
        case 'Auditorías':
              return(
                <Link key={key.urlMenu} href={key.urlMenu} >
                    <a>
                    <FcSurvey key={key.idMenu} className={classVar} >
                    </FcSurvey>
                    {key.description}
                    </a>
                </Link>
        ...
        case 'Personal Competente':
              return(
                 <Link key={key.urlMenu} href={key.urlMenu} >
                    <a>
                      <FcConferenceCall key={key.idMenu} className={classVar} >
                      </FcConferenceCall>
                      {key.description}
                      </a>
                </Link>
        ...
        case "Configuración iCalidad":
              return(
                  <Link key={key.urlMenu} href={key.urlMenu} >
                    <a>
                      <AiFillSetting key={key.idMenu} className={classVar} >
                      </AiFillSetting>
                      {key.description}
                      </a>
                    </Link>
        ...
        case "Catálogos":
              return(
                <Link key={key.urlMenu} href={key.urlMenu} >
                <a>
                  <MdTask key={key.idMenu} className={classVar} >
                  </MdTask>
                  {key.description}
                </a>
                </Link>
 ...
 function  SideNavBar  ({ idUser })  {
    ...
    const { data: menu, error, isLoading } =   useSWR(`/api/User/dasBoardUser/${idUser}/${idiCalidad}/${menuDataMenu.idMenuFather}`);
   ...
   return (
    <>
     <IconButton
        ...
        <Collapse in={isOpen} animateOpacity  className={` h-screen ${!isOpen ? "block" : "hidden" } bg-slate-700  `}>
            <div className={`menu w-70 p-0 rounded-tr-box rounded-br-box  outline-2 outline-black bg-red-400 `}>
                { menuArray?.map((item) => (
                  <li key={item.IdMenu} onClick={() => setDataMenu({idMenuFather: item.IdMenu, idMenu: 0})}>
                  <ElementMenu1  idMenu={item.IdMenu} description={item.Menu} 
                    urlMenu={item.URL} idMenuFather={item.IdMenuPadre} 
                     />
                  </li>
                )) }
            </div>
